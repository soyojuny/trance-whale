...
import {
  loadCatalogSource,
  type CatalogSourceDependencies,
} from "../../../../lib/source/catalog-source.server";
...
function errorResponse(error: unknown): Response {
  ...
  return Response.json(publicError, {
    status: STATUS_BY_ERROR[publicError.code] ?? 500,
...
export function createCatalogSourceHandler(dependencies: CatalogSourceDependencies = {}) {
  return async function catalogSourceHandler(request: Request): Promise<Response> {
    try {
      ...
      const catalog = await loadCatalogSource(requestData.data.url, dependencies);
...
export const POST = createCatalogSourceHandler();
