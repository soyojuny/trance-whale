...
- 작업 시작 시 해당 step의 명령에 필요한 `node`, `npm`, `npx`와 E2E 브라우저만 한 번 점검한다. 누락된 의존성·인증·수동 설정은 반복 재시도하지 말고 `blocked` 사유로 기록한다.
- 브라우저·저장소 문제는 실제 사용자 흐름을 먼저 재현하고, 확인된 사실·가설·다음 검증을 구분한다. 같은 조건의 실패한 접근은 반복하지 않는다.
- 외부 인증은 세션 지속 가능 여부와 최소 권한을 먼저 확인한다. 사용자 승인이 필요한 절차는 지속 가능한 단일 경로만 안내하며, 토큰·키는 출력·URL·Git 설정에 남기지 않는다.
- step에는 실제로 읽어야 할 문서와 파일만 명시한다. 전체 `docs/`를 다시 읽는 작업은 문서 감사처럼 전체 범위가 필요한 경우에만 한다.
- 탐색은 파일 목록과 필요한 코드 범위를 한 번에 수집한다. 전체 저장소 목록, 긴 파일 전문, 이미 확인한 명령 출력을 반복해서 읽지 않으며 도구 출력은 필요한 범위(기본 2,000 토큰 이하)로 제한한다.
- 장시간 명령은 예상 완료 시점에 한 번 확인한다. 상태 변화가 없는 빈 폴링을 연속으로 실행하지 않는다.
- 검증은 계층적으로 수행한다. 작업 중에는 변경한 영역의 테스트를 우선 실행하고, 논리적으로 묶인 변경이 끝난 뒤 `typecheck`와 `lint`를 한 번 실행한다. 전체 `test`, `build`, E2E는 phase 또는 사용자 흐름 완료 시 한 번 실행한다.
- 기존에 승인된 UI를 변경하는 step은 `docs/UI_GUIDE.md`의 해당 화면을 읽고, 360px와 데스크톱에서 의도한 차이를 확인한다. 아이콘 동작을 텍스트 동작으로 바꾸는 등 시각·상호작용 변경은 명시적인 요구 또는 승인 없이는 하지 않는다.
- UI 변경의 검증 결과와 의도한 차이는 `docs/VALIDATION.md`의 회귀 방지 기준에 따라 기록한다. 의도하지 않은 차이가 있으면 통합 전에 복구한다.
