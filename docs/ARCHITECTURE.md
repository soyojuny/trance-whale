...
- 매 시도마다 요청 문단에 전체 원문의 첫 문단이 포함되는지 `isChapterStart`로 전달한다. 기본 프롬프트 `v2`는 이 값으로 장 시작과 이어지는 본문을 구분하여 사용자 지시의 최상단 제목·시작 문구를 첫 문단 번역에만 한 번 적용하도록 안내한다. 말투·용어 지시와 원문에 실제로 있는 중간 제목은 모든 요청에서 유지한다. 재개나 부분 성공 후 재시도에서도 청크 번호 대신 남은 문단과 전체 원문의 첫 문단 ID를 비교한다.
